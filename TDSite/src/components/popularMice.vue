<template>
    <v-data-iterator :items="mice" :items-per-page="itemsPerPage" class="mr-8">
        <template v-slot:header="{ page, pageCount, prevPage, nextPage }">
            <h1 class="text-h4 font-weight-bold d-flex justify-space-between mb-4 align-center">
                ...
                <div class="d-flex align-center">
                    <v-btn class="me-8" prepend-icon="mdi-arrow-down"  variant="text" @click="onClickSeeAll">
            ...
            <v-row>
                <v-col v-for="(item, i) in items" :key="i" cols="12" sm="6" xl="3">
                    <v-card  variant="tonal">
                        ...
                        <ul class="mx-4 my-2 grid grow-cols-3 gap-2">
                            <li>DPI: {{ item.raw.dpi }}</li>
                            <li>Buttons: {{ item.raw.buttons }}</li>
                            <li>Weight: {{ item.raw.weight }}</li>
                            <li>Wireless: {{ item.raw.wireless ? 'Yes' : 'No' }}</li>
                            <li>Price: ${{ item.raw.price }}</li>
                        </ul>
                    </v-card>
                </v-col>
            </v-row>
        ...
        <template v-slot:footer="{ page, pageCount }">
            <v-footer color="transparent" class="justify-space-between text-body-2 my-4">
