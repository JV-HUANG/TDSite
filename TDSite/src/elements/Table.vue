      ...
      <tbody class="divide-y divide-border">
        <template v-if="table.getRowModel().rows.length">
          <tr v-for="row in table.getRowModel().rows" :key="row.id" :class="getRowClasses(row)"
            ...
            <td v-for="cell in row.getVisibleCells()" :key="cell.id" :class="getCellClasses()">
        ...
        <template v-else>
          <tr>
            <td :colspan="table.getAllColumns().length" :class="getCellClasses()" class="text-center py-4">
              No results found
            </td>
          </tr>
        </template>
