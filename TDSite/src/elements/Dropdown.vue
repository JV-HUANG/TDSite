...
<template>
  <div class="relative min-w-32 w-auto inline-block">
    ...
    <button @click="toggleDropdown"
      ...
      <svg class="w-4 h-4 ml-2" :class="{ 'transform rotate-180': isOpen }" fill="none" stroke="currentColor"
    ...
    <!-- Dropdown Menu -->
    <div v-if="isOpen" class="absolute z-10 w-full mt-1 bg-card rounded-md shadow-lg ring ring-border">
      <div class="py-1">
        <a v-for="(option, index) in options" :key="index" @click="selectOption(option)"
          class="block px-4 py-2 text-sm text-white hover:bg-background cursor-pointer"
          :class="{ 'bg-card': option === selectedOption }">
          <div class="flex items-center">
            <svg v-if="option === selectedOption" class="w-4 h-4 mr-2" fill="none" stroke="currentColor"
              viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
